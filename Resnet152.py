...
class ResNet152(nn.Module):
    def __init__(self, block):
        ...
        self.stage2 = self._make_stage(block, 128, 8, stride=2)
        # Residual Stage 3: 해상도를 다시 반으로 줄임
        self.stage3 = self._make_stage(block, 256, 36, stride=2)
...
if __name__ == '__main__':
    ...
    model = ResNet152(BottleneckBlock).to(device) # 모델을 GPU로 이동 
    ...
    def train_model(model, train_loader, val_loader, optimizer, criterion, epochs, max_patience):
        ...
        torch.save(best_model, 'model/Resnet152_bestmodel.pth') # 최적의 결과였던 모델 parameter를 저장함.
    ...
    ax[0].set_title('Resnet152_Validation Loss')
    ax[1].plot(range(1, len(losses)+1), accuracys)
    ax[1].set_xlabel('epoch')
    ax[1].set_ylabel('Validation Acc')
    ax[1].set_title('Resnet152_Validation Accuracy')
    plt.show()