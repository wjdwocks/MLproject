...
class ResNet50(nn.Module):
    def __init__(self, block):
        ...
        self.stage3 = self._make_stage(block, 256, 6, stride=2)
...
if __name__ == '__main__':
    ...
    def train_model(model, train_loader, val_loader, optimizer, criterion, epochs, max_patience):
        ...
        torch.save(best_model, 'model/Resnet50_bestmodel.pth') # 최적의 결과였던 모델 parameter를 저장함.
    ...
    ax[0].set_title('Resnet50_Validation Loss')
    ax[1].plot(range(1, len(losses)+1), accuracys)
    ax[1].set_xlabel('epoch')
    ax[1].set_ylabel('Validation Acc')
    ax[1].set_title('Resnet50_Validation Accuracy')
    plt.show()